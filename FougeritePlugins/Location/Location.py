...
class Location:
    ...
    def On_PluginInit(self):
        for x in d.keys():
            v = d[x].split(',')
            Vector2s[Util.CreateVector2(float(v[0]), float(v[1]))] = x
        d.clear()
        del d
    ...
    def On_Command(self, Player, cmd, args):
        if cmd == "location":
            ...
            v = None
            for x in Vector2s.keys():
                dist = Util.GetVector2sDistance(loc, x)
                if dist < closest:
                    v = x
                    closest = dist
            Player.MessageFrom("DetailedLocation", "You are near: " + Vector2s[v] + " - " + str(Player.Location))
